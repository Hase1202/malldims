...
from .models import (
    Item, Brand, Customer, Transaction, TransactionItem, 
    Account, CustomerBrandPricing, ItemTierPricing, CustomerSpecialPricing, ItemBatch
)
from .serializers import (
    ItemSerializer, BrandSerializer, CustomerSerializer,
    TransactionSerializer, TransactionCreateSerializer, TransactionItemSerializer,
    AccountSerializer, CustomerBrandPricingSerializer,
    ItemTierPricingSerializer, CustomerSpecialPricingSerializer, ItemBatchSerializer
)
...
class ItemViewSet(viewsets.ModelViewSet):
    ...
    def next_batch_number(self, request, pk=None):
        ...
        highest_batch = ItemBatch.objects.filter(
            item=item
    ...
    def history(self, request, pk=None):
        ...
        for trans_item in transaction_items:
            ...
            history_entry = {
                ...
                'customer_name': trans_item.transaction.customer.company_name if trans_item.transaction.customer else None,
                'brand_name': trans_item.transaction.brand.brand_name if trans_item.transaction.brand else None,
                ...
                } if trans_item.transaction.account else None
            }
    ...
    def special_pricing(self, request, pk=None):
        ...
        for pricing in special_pricing:
            pricing_data.append({
                ...
                } if pricing.created_by else None
            })
...
class CustomerViewSet(viewsets.ModelViewSet):
    ...
    def set_brand_pricing(self, request, pk=None):
        ...
        try:
            ...
            brand_pricing, created = CustomerBrandPricing.objects.update_or_create(
                customer=customer,
                brand=brand,
                defaults={'pricing_tier': pricing_tier}
            )
    ...
    def add_special_pricing(self, request, pk=None):
        ...
        try:
            ...
            if CustomerSpecialPricing.objects.filter(customer=customer, item=item).exists():
                return Response(
...
class TransactionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Transaction.objects.select_related('brand', 'customer', 'account').all()
        ...
        for tx in sample_transactions:
            print(f"Sample TX {tx.transaction_id}: DB_type='{tx.transaction_type}', is_released={tx.is_released}, is_paid={tx.is_paid}, is_or_sent={tx.is_or_sent}")
        ...
        if status:
            ...
            if status.lower() == 'pending':
                ...
                # Handle both old and new transaction type formats
                incoming_filter = Q(transaction_type='INCOMING') | Q(transaction_type='Receive Products')
                adjustment_filter = Q(transaction_type='ADJUSTMENT') | Q(transaction_type='Manual correction')
                ...
                queryset = queryset.filter(incoming_filter | adjustment_filter | outgoing_completed_filter)
        ...
        if transaction_type:
            ...
            elif transaction_type in ['Manual correction', 'Manual Adjustment', 'Adjustment']:
        ...
        if release_status:
            if release_status.lower() == 'released':
                queryset = queryset.filter(transaction_type='OUTGOING', is_released=True)
            elif release_status.lower() == 'not released':
                queryset = queryset.filter(transaction_type='OUTGOING', is_released=False)
        ...
        if or_invoice_status:
            if or_invoice_status.lower() == 'sent':
                ...
                queryset = queryset.filter(transaction_type='OUTGOING', is_or_sent=False)
...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Transaction counts
            pending_outgoing = Transaction.objects.filter(
                transaction_type='OUTGOING'
            ).exclude(
                is_released=True, is_paid=True, is_or_sent=True
            ).count()
...
class PricingUtilsView(APIView):
    ...
    def get(self, request):
        ...
        return Response({
            'pricing_tiers': [{'value': tier[0], 'label': tier[1]} for tier in tiers]
        })
    
    def post(self, request):
        """Calculate price for customer and item"""
...
class ItemBatchViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = ItemBatch.objects.select_related('item', 'item__brand', 'transaction').all()
