...
from .models import (
    ...
    CustomerBrandPricing, ItemTierPricing, CustomerSpecialPricing, ItemBatch
)
...
class ItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'sku', 'uom', 'uom_display', 'total_quantity', 'active_batches_count',
            'created_at', 'updated_at', 'tier_pricing'
        ]
        read_only_fields = ['item_id', 'created_at', 'updated_at', 'brand_name', 'brand_id', 'sku', 'total_quantity', 'active_batches_count']
...
class TransactionItemSerializer(serializers.ModelSerializer):
    ...
    batch_remaining_quantity = serializers.IntegerField(source='batch.remaining_quantity', read_only=True)
...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    def get_transaction_status(self, obj):
        ...
        if obj.transaction_type in ['INCOMING', 'ADJUSTMENT']:
            return 'Completed'  # INCOMING and ADJUSTMENT transactions are always completed
        else:  # OUTGOING
            if obj.is_completed:
                return 'Completed'
            else:
...
class TransactionCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'due_date', 'reference_number', 'notes', 'items'
        ]
    ...
    def create(self, validated_data):
        ...
        is_adjustment = getattr(self, 'original_transaction_type', None) in ['Manual correction', 'Manual Adjustment', 'Adjust Inventory'] or validated_data.get('transaction_type') == 'ADJUSTMENT'
        
        if is_adjustment and not validated_data.get('brand'):
            if items_data:
                first_item_id = items_data[0].get('item') or items_data[0].get('item_id')
                if first_item_id:
                    try:
        ...
        for item_data in items_data:
            ...
            if item_id:
                try:
                    ...
                    quantity_change = item_data.get('quantity_change') or item_data.get('quantity', 0)
                    quantity = abs(quantity_change)  # Absolute value for transaction item record
                    ...
                    if transaction.transaction_type == 'OUTGOING':
                        ...
                        if batch_id:
                            try:
                                ...
                                raise serializers.ValidationError(f"Batch with ID {batch_id} does not exist.")
                    ...
                    if transaction.transaction_type == 'INCOMING':
                        ...
                        if quantity_change > 0:
                            ...
                            ItemBatch.objects.create(
                                item=item,
                                cost_price=unit_price or 0,  # Use provided price or 0 for adjustments
                                initial_quantity=quantity,
                                remaining_quantity=quantity,
                                transaction=transaction
                            )
